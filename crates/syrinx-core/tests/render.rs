...
const SINE: &str = r#"
import { Osc, render } from "syrinx";
export const meta = { name: "sine", duration: 0.1, channels: 1 };
export const stems = {
  sine(ctx) {
...
fn same_source_same_bytes() {
    let src = r#"
import { Noise, Reverb, Biquad, Env, render, filter } from "syrinx";
export const meta = { name: "n", duration: 0.5, channels: 2, seed: 99 };
export const stems = {
  noise(ctx) {
...
"#;
    let a = render(src, "n.syr", &opts()).unwrap();
    let b = render(src, "n.syr", &opts()).unwrap();
...
const TWO: &str = r#"
import { render } from "syrinx";
export const meta = { name: "two", duration: 0.01, channels: 1 };
export const stems = {
...
fn mix_is_bit_identical_to_the_sum_of_its_stems() {
    let mix = render(TWO, "two.syr", &opts()).unwrap();
    let each = render_each(TWO, "two.syr", &opts(), &[]).unwrap();
...
fn a_subset_is_summed_without_the_mix_stage() {
    let src = r#"
import { render } from "syrinx";
export const meta = { duration: 0.01, channels: 1 };
export const stems = {
  a(ctx) { return render(ctx, () => 0.25); },
  b(ctx) { return render(ctx, () => 0.125); },
};
...
fn the_mix_stage_receives_planes_per_stem() {
    let src = r#"
import { render } from "syrinx";
export const meta = { duration: 0.01, channels: 2 };
export const stems = {
  a(ctx) { return render(ctx, () => 0.25); },
};
...
fn stems_are_isolated_from_each_other() {
    let src = r#"
let calls = 0;
export const meta = { duration: 0.001, channels: 1 };
export const stems = {
...
"#;
    let head = "export const meta = { duration: 0.001, channels: 1 };\nexport const stems = {";
...
fn sample_rate_override_and_declared_rate() {
    let src = r#"
export const meta = { duration: 1, sampleRate: 22050 };
...
fn name_defaults_to_file_stem() {
    ...
    assert_eq!(info.meta.name, "thing");
...
#[test]
fn api_pin_is_enforced() {
    let ok = "export const meta = { duration: 0.01, api: 2 };\nexport const stems = { a: (ctx) => [] };";
    assert!(inspect(ok, "x.syr", &opts()).is_ok());
    let bad = "export const meta = { duration: 0.01, api: 1 };\nexport const stems = { a: (ctx) => [] };";
    let e = inspect(bad, "x.syr", &opts()).unwrap_err();
    assert_eq!(e.kind, ErrorKind::Contract);
    assert!(e.message.contains("api 1"), "{}", e.message);
}

#[test]
fn rejects_math_random_before_running() {
    let src = "import { render } from \"syrinx\";\nexport const meta = { duration: 0.01 };\nexport const stems = {\n  a(ctx) {\n    return render(ctx, () => Math.random());\n  },\n};";
    let e = render(src, "x.syr", &opts()).unwrap_err();
    assert_eq!(e.kind, ErrorKind::Check);
    let (line, column) = position_of(src, "Math.random");
    assert_eq!((e.file.as_deref(), e.line, e.column), (Some("x.syr"), Some(line), Some(column)));
}
...
fn the_timeout_is_a_budget_for_the_whole_render() {
    let mut src = String::from("export const meta = { duration: 0.01, channels: 1 };\nexport const stems = {\n");
    for i in 0..256 {
...
fn contract_errors() {
    let cases: &[(&str, &str)] = &[
        ...
        ("export const meta = { duration: 0.01 };", "`stems` export"),
        ...
        ("export const meta = { duration: -1 };\nexport const stems = { a: (ctx) => [] };", "duration"),
        ("export const meta = { duration: 0.01, channels: 3 };\nexport const stems = { a: (ctx) => [] };", "channels"),
        (
...
export const meta = { duration: 0.05 };
export const stems = {
  beep(ctx) {
...
"#;
    ...
    expected.sort();
    assert_eq!(deps, expected, "diamond import must load tone.js once");

    assert_eq!(inspect(src, entry.to_str().unwrap(), &opts()).unwrap().dependencies.len(), 2);
}
...
fn import_outside_root_is_rejected() {
    ...
    assert_eq!(e.kind, ErrorKind::Contract);
    assert!(e.message.contains("outside the project root"), "{}", e.message);
...
fn bare_specifiers_and_missing_files_are_errors() {
    ...
    assert!(e.message.contains("only \"syrinx\" and relative paths"), "{}", e.message);

    let missing = "import x from \"./nope.js\";\nexport const meta = { duration: 0.01 };\nexport const stems = { a: () => [] };";
    let e = render(missing, entry.to_str().unwrap(), &opts()).unwrap_err();
    assert!(e.message.contains("cannot import \"./nope.js\""), "{}", e.message);
}
...
fn renders_concurrently_from_many_threads() {
    let handles: Vec<_> = (0..8)
        .map(|_| std::thread::spawn(|| render(SINE, "sine.syr", &opts()).unwrap().samples))
        .collect();
