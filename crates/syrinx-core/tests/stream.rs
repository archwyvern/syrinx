...
fn the_live_path_equals_the_whole_path() {
    let live = render(STREAMED, &in_project("s.syr"), &opts()).unwrap();
    let each = render_each(STREAMED, &in_project("s.syr"), &opts(), &[]).unwrap();
    let supplied: Vec<(String, Vec<f32>)> = each.iter().map(|r| (r.stem.clone().unwrap(), r.samples.clone())).collect();
    let whole_path = mix_from(STREAMED, &in_project("s.syr"), &opts(), &supplied).unwrap();
    assert_eq!(live.samples, whole_path.samples);

    // And both equal the same sound written entirely in the api-2 forms.
    let api2 = render(WHOLE, &in_project("s.syr"), &opts()).unwrap();
    assert_eq!(live.samples, api2.samples);
    assert_eq!(live.frames, 24_000);
    assert!(live.peak() > 0.1);

    let mut stream = Stream::open(STREAMED, &in_project("s.syr"), &opts()).unwrap();
...
fn the_block_sum_equals_the_whole_sum() {
    ...
    let a = render(streamed, &in_project("s.syr"), &opts()).unwrap();
    let b = render(whole, &in_project("s.syr"), &opts()).unwrap();
    assert_eq!(a.samples, b.samples);

    // A subset of streaming layers sums the same way, without the mix stage.
    let sub = render(STREAMED, &in_project("s.syr"), &only(&["tone", "hits"])).unwrap();
    let each = render_each(STREAMED, &in_project("s.syr"), &opts(), &["tone".into(), "hits".into()]).unwrap();
...
"#;
    let r = render(src, &in_project("x.syr"), &opts()).unwrap();
    assert_eq!(r.frames, 9_600);
    assert_eq!(r.samples[7], 0.2);
    let mut s = Stream::open(src, &in_project("x.syr"), &opts()).unwrap();
    assert!(s.streaming());
...
fn the_four_forms() {
    ...
    let head = "export const meta = { api: 4, duration: 0.2, channels: 1 };\n";
    for (layer, mix, layer_streams, mix_streams) in [
        ...
        let mut stream = Stream::open(&src, &in_project("x.syr"), &opts()).unwrap();
...
"#;
    ...
    assert!(!Source::open(src, &in_project("x.syr"), &opts()).unwrap().mixer(&Target::Mix).unwrap().streaming());
}
...
"#;
    ...
    assert_eq!(m.meta.name, None, "no name declared, none reported");
...
fn mono_blocks_are_widened_and_the_plane_count_is_checked() {
    let src = r#"
export const meta = { api: 4, duration: 0.1, channels: 2 };
...
fn a_non_finite_block_sample_names_its_absolute_frame() {
    let src = r#"
export const meta = { api: 4, duration: 0.2, channels: 2 };
...
fn restart_reproduces_blocks_for_a_stateless_mix() {
    let src = r#"
export const meta = { api: 4, duration: 0.5, channels: 1 };
...
"#;
    ...
    assert_eq!(all.samples, render(src, &in_project("x.syr"), &opts()).unwrap().samples);
...
"#;
    let forward = render(src, &in_project("x.syr"), &only(&["big", "tiny1", "tiny2"])).unwrap();
    let backward = render(src, &in_project("x.syr"), &only(&["tiny2", "tiny1", "big"])).unwrap();
...
fn dropping_a_stream_stops_a_spinning_layer() {
    let src = r#"
export const meta = { api: 4, duration: 2, channels: 1 };
...
fn the_source_reports_what_it_opened() {
    let source = Source::open(STREAMED, &in_project("s.syr"), &opts()).unwrap();
    assert_eq!(source.meta().name.as_deref(), Some("s"));
