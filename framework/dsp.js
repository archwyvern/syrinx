...
//
// normalize, fade and place need the whole render, so inside a stream's block they refuse and say
// what to do instead. A layer may still call them while it sets up, before it returns its stream.

import { Random, inBlock } from "syrinx";
...
// The helpers below need the whole render. Inside a stream's block they refuse, naming what to
// do instead; the core says whether a block is being computed.
...
export {
  TAU,
  clamp, lerp, db, mtof, softclip, hardclip, fold,
  Phasor, Osc, BlepOsc,
  Noise,
  Env,
  OnePole, Biquad, Svf,
