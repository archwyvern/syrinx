...
syrinx 0.9.0: contract version 4 (a source declares `meta.api` 4), streams in blocks of 4096 frames.

## The core module

What a conforming host must know about (SPEC.md, clause 12): `import ... from "syrinx"`.
...
- `api: number` -- Contract version the source was written against: 4. Required; any other is refused.
- `name?: string` -- Display name. No default: a source that declares none has none.
- `duration: number` -- Length in seconds, in (0, 600]. Required.
- `channels?: 1 | 2` -- 1 (default) or 2. A mono return is duplicated when this is 2.
- `sampleRate?: number` -- Preferred sample rate, an integer in [8000, 192000]; a host may override it. Default 48000.
- `seed?: number` -- An integer in [0, 4294967295], handed to every layer and to the mix as `ctx.seed`. Default 0.
- `loop?: boolean` -- Declared seamless loop. Passed through to consumers; it changes no sample.
